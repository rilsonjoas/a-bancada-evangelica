...
//
// theme: slug do tema legislativo — precisa bater com o catálogo do front
// (src/lib/themes.tsx). Agrupa as proposições nas páginas "como votaram
// sobre X" (M2).
...
async function main() {
  console.log(`\n🗂️  ${IMPACTS.length} proposições com impacto leigo + tema cadastrada(da)s`);
  ...
  for (const impact of IMPACTS) {
    const agendas = await prisma.keyAgenda.findMany({
      ...
      select: { id: true, title: true, practical_impact: true, theme: true },
    });
    ...
    for (const a of agendas) {
      if (a.practical_impact === impact.text && a.theme === impact.theme) continue;
      await prisma.keyAgenda.update({
        where: { id: a.id },
        data: { practical_impact: impact.text, theme: impact.theme },
      });
      updated++;
    }
    console.log(`   ✓ ${impact.prefix}: ${agendas.length} agenda(s) → tema + texto aplicados`);
  }
  ...
  const withTheme = await prisma.keyAgenda.count({ where: { theme: { not: null } } });
  console.log(`🔍 ${withText} agendas ativas com impacto leigo · ${withTheme} com tema`);
}
